 ...
 */
class Matrix_date_no_shift_ft {

	var $info = array(
		'name' => 'Date (no shift)'
	);
    // From EE Localize.php

	var $server_now			= '';	// Local server time
	var $now				= 0;  // Local server time as GMT
	...
	function __construct()
	{
		...
		if (! isset($this->EE->session->cache['matrix']['celltypes']['date_no_shift']))
		{
			$this->EE->session->cache['matrix']['celltypes']['date_no_shift'] = array();
		}
		$this->cache =& $this->EE->session->cache['matrix']['celltypes']['date_no_shift'];
	...
	function display_cell($data)
	{
		if (! isset($this->cache['displayed']))
		{
			...
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="'.$theme_url.'scripts/matrix_date_no_shift.js"></script>');
	...
	function save_cell($data)
	{
		...
		return $this->convert_human_date_to_gmt($data);
	}
	...
	function replace_tag($data, $params = array())
	{
		...
		{
			$data = $this->EE->localize->set_human_time($data, false);
		}
    ...
    // THE REST OF THESE FUNCTIONS ARE FROM Localize.php
	...
	function convert_human_date_to_gmt($datestr = '')
	{
		if ($datestr == '')
		{
			...
			else
			{
				...
				$sec = date('s', $this->now);
                                ...
                                // removing this shift EE-211 -- Josh K
				//$fib_seconds = TRUE;
			}
		...
		// Daylight saving time.
                ...
                // Removing this time shift EE-211 -- Josh K
		...
		if (date("I", $this->now))
		{
			...
			{
				$time -= 3600;
                                file_put_contents('/home/josh/schenanigans', "case a\n", FILE_APPEND);
			}
		}
		else
		{
			...
			{
				$time += 3600;
                                file_put_contents('/home/josh/schenanigans', "case b\n", FILE_APPEND);
			}
		}
                */
	...
	function decode_date($datestr = '', $unixtime = '', $localize = TRUE)
	{
		...
		if ($localize === TRUE)
		{
			$unixtime = $this->set_localized_time($unixtime,'','n');
			$prelocalized = TRUE;
		}
